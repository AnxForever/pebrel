...
| CODEX-SPARKLE | 对照终端 中可见的 Codex 星点在 Pebrel 不显示 | 核对真彩色能力、OSC 10/11 回答、盲文点字与前/背景色；真实 PTY 录制/回放；关闭动画时不强制产生动画 | 相同 Codex/模型/设置，对照 WT；启动、输入、弹窗遮挡、焦点、主题切换；不能伪造 `WT_SESSION` 来冒充 WT | 完整 Windows 子环境漏真彩色声明已修复；8 项环境回归及 15 主题 VT 回放通过；真实产品星点字形/背景截图已核对；真实 Codex 动画仍需验收 |
