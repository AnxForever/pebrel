...
- Added persistent command groups, with drag-and-drop assignment and context-menu actions to move commands between groups or return them to Ungrouped. Deleting a group keeps its commands.
...
- Fixed WSL repeatedly printing `PROMPT_COMMAND` syntax errors at `__pebrel_connection() {` when Windows line endings entered embedded Shell scripts. Shared scripts are normalized before injection, preserving command exit codes and existing user SSH functions.
...
- Fixed hook-enabled AI sessions showing a completion toast merely because the screen stopped changing while the task was still working.
...
- Fixed moving the final tab to another window triggering a conflicting workspace update when the original window closes.
- Fixed saved SSH connection names being replaced by remote directory or program titles. Both the sidebar and top-tab layouts retain the configured connection name.
...
- 新增：可持久保存的命令分组，支持拖动分配、右键移入其他分组或移回“未分组”；删除分组会保留其中的命令。
- 新增：内置命令配方支持持久删除；重新打开命令管理器或从另一窗口修改后，被删除的配方仍保持隐藏，用户保存的命令予以保留。

#### 修复

- 修复：Windows 换行符混入内置 Shell 脚本后，WSL 在 `__pebrel_connection() {` 处反复输出 `PROMPT_COMMAND` 语法错误的问题。共享脚本在注入前统一换行，保留命令退出码及用户已有的 SSH 函数。
...
- 修复：已启用 Hook 的 AI 会话仍在工作时，仅因屏幕暂时没有变化就弹出完成提示的问题。
...
- 修复：将最后一个标签移动到另一窗口、原窗口随之关闭时触发工作区更新冲突的问题。
- 修复：已保存的 SSH 连接名称被远端目录或程序标题替换的问题；侧栏和顶部标签布局均保留配置中的连接名称。
...
- [@AnxForever](https://github.com/AnxForever) — [#142](https://github.com/Kuddev/pebrel/pull/142), [#143](https://github.com/Kuddev/pebrel/pull/143), [#145](https://github.com/Kuddev/pebrel/pull/145)
- [@AlanWanco](https://github.com/AlanWanco) — [#166](https://github.com/Kuddev/pebrel/pull/166)
- [@Traveritas](https://github.com/Traveritas) — [#168](https://github.com/Kuddev/pebrel/pull/168)

---

**SHA256**

- `Pebrel-v1.8.2-windows-x64.zip`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-windows-x64-setup.exe`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-linux-x64-preview.AppImage`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-linux-x64-preview.deb`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-linux-x64-preview.tar.gz`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-macos-arm64-preview.dmg`: `PENDING FINAL BUILD`
- `Pebrel-v1.8.2-macos-x64-preview.dmg`: `PENDING FINAL BUILD`
