...
// agent_end 只结束一次尝试；Pi 0.80.4+ 的 agent_settled 才覆盖自动重试。
// 旧版仍使用原有结束边界。无 hook 环境变量时，扩展不启动子进程。
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    outcome = last?.stopReason === "stop" ? "success"
      : last?.stopReason === "error" ? "failed"
      : last?.stopReason === "aborted" ? "cancelled" : "unknown";
